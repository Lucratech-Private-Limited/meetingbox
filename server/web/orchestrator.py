...
# Reuse one OpenAI client per worker for the intent router / multi-agent planner.
# Constructing a fresh OpenAI() per call pays a TLS/HTTP handshake (~0.5s and
# occasional multi-second cold spikes) on every routed turn. The SDK client is
# thread-safe, so caching it removes that per-call handshake from the hot path.
# (Mirrors the cached realtime client in routes/voice.py.)
_openai_router_client = None
_openai_router_client_lock = threading.Lock()


def _get_openai_router_client():
  """Return a cached OpenAI client for routing/planning, or None if no API key."""
  global _openai_router_client
  c = _openai_router_client
  if c is not None:
    return c
  key = os.getenv("OPENAI_API_KEY", "").strip()
  if not key:
    return None
  with _openai_router_client_lock:
    if _openai_router_client is None:
      from openai import OpenAI

      _openai_router_client = OpenAI(api_key=key)
    return _openai_router_client
...
def _route_llm_openai(
  ...
  client = _get_openai_router_client()
  if client is None:
    return None
  try:
