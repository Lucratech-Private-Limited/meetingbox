...
async def delete_account(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Remove meetings (and their children) plus on-disk audio files.
        cur.execute("SELECT id, audio_path FROM meetings WHERE user_id = ?", (user_id,))
        meeting_rows = cur.fetchall()
        for row in meeting_rows:
            mid, audio_path = row[0], row[1]
            ...
            cur.execute("DELETE FROM processing_state WHERE meeting_id = ?", (mid,))
            if audio_path:
                try:
                    Path(audio_path).unlink(missing_ok=True)
                except OSError:
                    logger.warning("Could not delete audio file during account deletion: %s", audio_path)
        cur.execute("DELETE FROM meetings WHERE user_id = ?", (user_id,))

        # Remove all other user-scoped rows.
        for table in (
            "devices",
        ...
        cur.execute(
            "DELETE FROM admin_memory_access_log WHERE admin_user_id = ? OR target_user_id = ?",
            (user_id, user_id),
        )
    ...
    # Best-effort: clear the vector/memory store. Never block deletion on this.
    try:
        from services.mem0_service import delete_user_memories

        delete_user_memories(user_id)
    except Exception as exc:
        logger.warning("Could not delete mem0 memories for user %s: %s", user_id, exc)

    logger.info("Deleted account and all data for user %s", user_id)
