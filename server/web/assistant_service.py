...
def plan_calendar_steps(message: str) -> list[dict[str, Any]]:
  ov = single_call_router.pop_override("calendar_agent")
  if ov is not None:
    return ov
...
def _resolve_attendee_tokens_to_emails(user_id: str, values: list[Any]) -> tuple[list[str], set[str]]:
  resolved = _normalize_emails(values)
  ...
  for value in values:
    ...
    if "@" in token:
      continue
    norm = _normalize_name_token(token)
    if not norm:
      continue
    try:
      matches = lookup_contacts(user_id, norm, limit=5)
    except Exception:
      logger.debug("lookup_contacts failed during attendee token resolve", exc_info=True)
      matches = []
    if not matches:
      unresolved_names.add(norm)
      continue
    existing = {r.lower() for r in resolved}
    for m in matches:
      em = str((m.get("email") if isinstance(m, dict) else "") or "").strip()
      if em and em.lower() not in existing:
        resolved.append(em)
        existing.add(em.lower())
  return resolved, unresolved_names


def _normalize_emails(values: list[Any]) -> list[str]:
  out: list[str] = []
  for value in values:
    ...
    if "@" not in s:
      continue
    key = s.lower()
    if key not in {v.lower() for v in out}:
      out.append(s)
  return out


def _infer_calendar_date_hint(message: str, args: dict[str, Any]) -> str | None:
  raw = str(args.get("date") or args.get("date_hint") or "").strip()
...
def _infer_calendar_time_hint(message: str, args: dict[str, Any]) -> str | None:
  raw = str(args.get("time") or args.get("time_hint") or "").strip()
  if raw:
    ...
    ap = re.fullmatch(r"(\d{1,2})(?::(\d{2}))?\s*(am|pm)", raw, flags=re.I)
    if ap:
      hh = int(ap.group(1))
      mm = int(ap.group(2) or "0")
      ampm = ap.group(3).lower()
  ...
  start_raw = str(args.get("start_time") or args.get("start_iso") or "").strip()
  if start_raw:
    sm = re.search(r"T(\d{2}):(\d{2})", start_raw)
    if sm:
      return f"{sm.group(1)}:{sm.group(2)}"

  mm = re.search(r"\b(?:at\s+)?(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b", message, flags=re.I)
  if mm:
    hh = int(mm.group(1))
    mn = int(mm.group(2) or "0")
    ampm = mm.group(3).lower()
    if ampm == "pm" and hh < 12:
      hh += 12
    elif ampm == "am" and hh == 12:
      hh = 0
    if 0 <= hh <= 23 and 0 <= mn <= 59:
      return f"{hh:02d}:{mn:02d}"

  return None


def _is_bulk_attendee_remove_request(message: str, args: dict[str, Any]) -> bool:
  removes = args.get("attendees_remove")
  ...
  return any(marker in m for marker in bulk_markers)


def _expand_bulk_attendee_remove_payloads(
  *,
  user_id: str,
  message: str,
  base_args: dict[str, Any],
) -> list[dict[str, Any]]:
  """Expand a broad attendee-removal request into per-event event_id updates."""
  raw_remove_values = list(base_args.get("attendees_remove") or [])
  if not raw_remove_values:
    raw_remove_values = _extract_emails_from_text(message)
  if not raw_remove_values:
    return []

  remove_emails, unresolved_names = _resolve_attendee_tokens_to_emails(user_id, raw_remove_values)
  remove_name_tokens = {_normalize_name_token(n) for n in unresolved_names if _normalize_name_token(n)}
  if not remove_emails and not remove_name_tokens:
    return []

  remove_set = {e.lower() for e in remove_emails}

  date_hint = _infer_calendar_date_hint(message, base_args)
  events_res = calendar_list_upcoming(
    user_id,
    max_results=80,
    date=date_hint or None,
    days_ahead=1 if date_hint else 1,
  )
  events = list(events_res.get("events") or [])
  if not events:
    return []
  ...
  for ev in events:
    ...
    for a in attendees:
      if isinstance(a, dict):
        ...
        nm = _normalize_name_token(a.get("name") or a.get("displayName") or "")
      else:
        em = str(a or "").strip().lower()
        nm = ""
      if em:
        attendee_emails.add(em)
      if nm:
        attendee_names.add(nm)

    email_hit = bool(remove_set and (attendee_emails & remove_set))
    name_hit = bool(remove_name_tokens and (attendee_names & remove_name_tokens))
    if not (email_hit or name_hit):
      continue

    payload = dict(base_args)
    payload["event_id"] = ev_id
    payload["attendees_remove"] = remove_emails
    summary = str(ev.get("summary") or "").strip()
    if summary:
      payload["title"] = summary
    start_obj = ev.get("start") if isinstance(ev.get("start"), dict) else {}
    start_raw = str(start_obj.get("dateTime") or start_obj.get("date") or "").strip()
    if start_raw:
      payload["date"] = start_raw[:10]
...
def plan_communication_steps(message: str) -> list[dict[str, Any]]:
  ov = single_call_router.pop_override("communication_agent")
  if ov is not None:
    return ov
...
def plan_memory_steps(message: str) -> list[dict[str, Any]]:
  ov = single_call_router.pop_override("memory_agent")
  if ov is not None:
    return ov
...
def plan_research_steps(message: str) -> list[dict[str, Any]]:
  ov = single_call_router.pop_override("research_agent")
  if ov is not None:
    return ov
  ...
  for t in tool_results:
    ...
    if tool == TOOL_MEMORY_SEARCH:
      ...
      if r.get("error"):
        ...
        continue
      title = r.get("title", "")
      kind = (r.get("session_type") or r.get("recording_mode") or "meeting")
      lines.append(f"{'Note' if kind == 'note' else 'Meeting'}: {title}")
      # Always include when it was recorded so the assistant can tell the user.
      when_date = (r.get("date") or "").strip()
      when_time = (r.get("time") or "").strip()
      if when_date or when_time:
        lines.append(f"Recorded: {when_date}{(' at ' + when_time) if when_time else ''}".strip())
      parts_ppl = r.get("participants")
      if isinstance(parts_ppl, list) and parts_ppl:
        lines.append(f"Participants: {', '.join(str(p) for p in parts_ppl)}")
      tags = r.get("tags")
      if isinstance(tags, list) and tags:
        lines.append(f"Tags: {', '.join(str(t) for t in tags)}")
      intent = (r.get("session_intent") or "").strip()
      if intent:
        lines.append(f"Captured intent: {intent}")
...
def _memory_fallback_reply(tool_results: list[dict[str, Any]]) -> str:
  ...
  for t in tool_results:
    ...
    if tool == TOOL_MEMORY_SEARCH:
      ...
      when_time = (r.get("time") or "").strip()
      if when_date or when_time:
        head += f" — recorded {when_date}{(' at ' + when_time) if when_time else ''}"
      ppl = r.get("participants")
      if isinstance(ppl, list) and ppl:
        head += f" (with {', '.join(str(p) for p in ppl)})"
      summ = (r.get("summary") or "").strip()
      if summ:
...
def _dispatch_single_agent(
  *,
  ...
  if agent_id == "calendar_agent":
    ...
    for step in steps:
      ...
      if tool == TOOL_CAL_LIST:
        ...
        if not user_id:
          ...
          continue
        try:
          inferred_time = _infer_calendar_time_hint(text, args)
          if inferred_time:
            args = dict(args)
            args["time_hint"] = inferred_time
        except Exception:
          logger.debug("delete time-hint infer failed", exc_info=True)
        pid = str(uuid.uuid4())
        pending_rows.append((pid, agent_id, tool, args))
        pending_meta.append({
          "id": pid,
        ...
        if not user_id:
          ...
          continue
        # For requests like "remove X from all meetings today", pre-resolve each
        # target event and queue explicit event_id updates to avoid title collisions
        # (e.g. two "Birthday Party" events on the same day).
        expanded_payloads: list[dict[str, Any]] = []
        try:
          if _is_bulk_attendee_remove_request(text, args):
            expanded_payloads = _expand_bulk_attendee_remove_payloads(
              user_id=user_id,
              message=text,
              base_args=args,
            )
          ...
          logger.exception("bulk attendee-remove expansion failed")
          expanded_payloads = []
        if expanded_payloads:
          for payload in expanded_payloads:
            pid = str(uuid.uuid4())
            pending_rows.append((pid, agent_id, tool, payload))
            pending_meta.append({
              "id": pid,
              "tool_name": tool,
              "status": "pending",
    ...
    # If retrieval is ambiguous (several plausible recordings), ask the user to
    # disambiguate instead of guessing or returning the most recent one.
    clarification = None
    if search_hit:
      sr = search_hit["result"]
      if sr.get("needs_clarification") and sr.get("clarification"):
        clarification = sr["clarification"]

    if clarification:
      assistant_lines.append(clarification.get("message") or "Which recording did you mean?")
    else:
      if search_hit:
...
def process_assistant_intent(
  *,
  ...
  # Opt-in single-call route+plan: ONE function-calling decision replaces the
  # router + per-agent planner round-trips. Steps are stashed in a thread-local
  # override consumed by plan_*_steps; the dispatch/execution path is unchanged.
  # Any failure (no tool, multi-agent, error) falls back to the legacy router.
  route: RouteResult | None = None
  single_call_router.clear_overrides()
  if text and single_call_router.single_call_enabled():
    try:
      planned = single_call_router.route_and_plan(text)
    except Exception:
      logger.exception("single-call router crashed; falling back to legacy router")
      planned = None
    if planned:
      sc_agent, sc_steps, sc_rationale = planned
      single_call_router.set_override(sc_agent, sc_steps)
      route = RouteResult(agent_id=sc_agent, method="single_call", rationale=sc_rationale)
  ...
  if multi_agent_enabled() and route.method != "single_call":
    try:
...
def approve_pending_action(pending_id: str, user_id: str) -> dict[str, Any]:
  ...
  try:
    if tool == TOOL_CAL_CREATE:
      ...
      try:
        from services.mem0_service import maybe_ingest_calendar_snapshot
        event_data = result if isinstance(result, dict) else {}
        maybe_ingest_calendar_snapshot(user_id, {"events": [event_data], "count": 1})
      except Exception:
        logger.debug("calendar create mem0 ingest failed", exc_info=True)
    elif tool == TOOL_CAL_DELETE:
      ...
      try:
        ...
        logger.debug("calendar update mem0 ingest failed", exc_info=True)
    elif tool == TOOL_CAL_RSVP:
      ...
      try:
        ...
        logger.debug("calendar rsvp mem0 ingest failed", exc_info=True)
    elif tool == TOOL_GMAIL_SEND:
      result = gmail_send_from_payload(user_id, payload)
      result_blob = {"gmail": result}
      try:
        from services.mem0_service import maybe_ingest_gmail_snapshot
        to_addr = str(payload.get("to") or "").strip()
        subject = str(payload.get("subject") or "").strip()
        cc_addr = str(payload.get("cc") or "").strip()
        bcc_addr = str(payload.get("bcc") or "").strip()
        body_preview = str(payload.get("body") or "")[:500].strip()
        if to_addr:
          msg: dict = {
            "to": to_addr,
            "subject": subject,
          ...
          maybe_ingest_gmail_snapshot(user_id, {"messages": [msg], "count": 1})
      except Exception:
        logger.debug("sent email mem0 ingest failed", exc_info=True)
    elif tool == TOOL_GMAIL_SEND_DRAFT:
      result = gmail_send_draft_from_payload(user_id, payload)
      result_blob = {"gmail_send_draft": result}
      try:
        from services.mem0_service import maybe_ingest_gmail_snapshot
        # send_draft result carries pre-send envelope recipients: [to_str, cc_str, bcc_str]
        recipients = result.get("recipients") or []
        to_addr = str(recipients[0]).strip() if len(recipients) > 0 else ""
        cc_addr = str(recipients[1]).strip() if len(recipients) > 1 else ""
        bcc_addr = str(recipients[2]).strip() if len(recipients) > 2 else ""
        if to_addr:
          msg_d: dict = {
            "to": to_addr,
            "direction": "sent",
            "snippet": (
              f"Sent draft email — To: {to_addr}"
              + (f", CC: {cc_addr}" if cc_addr else "")
            ),
          }
          if cc_addr:
          ...
          if bcc_addr:
            msg_d["bcc"] = bcc_addr
          maybe_ingest_gmail_snapshot(user_id, {"messages": [msg_d], "count": 1})
      except Exception:
        logger.debug("sent draft email mem0 ingest failed", exc_info=True)
