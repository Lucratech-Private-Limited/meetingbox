...
def memory_search_meetings(
  ...
  Optional filters (Fix 2):
    date_from  -- ISO date string YYYY-MM-DD; only meetings on/after this date.
    date_to    -- ISO date string YYYY-MM-DD; only meetings on/before this date.
    participant -- Name or email fragment; only meetings where participants column
                   contains this string (case-insensitive LIKE match).
  """
