...
**Files:** `docker-compose.prod.yml`, `scripts/install_device_ui.sh`,
...
**Files:** Configure manually on the host; `install_device_ui.sh` does not write `Xwrapper.config` or setuid `Xorg.wrap`.
...
Remove stale configs manually on the appliance if Xorg fails to start.

**Files:** (none in-repo — kiosk installers that deleted this were removed)
...
**Files:** Optional local unit `meetingbox-x.service` (historically generated by a removed production script, not by `install_device_ui.sh`)
...
`install_device_ui.sh` adds: `usermod -aG docker,video,input,audio,tty`.
...
**Files:** `scripts/install_device_ui.sh`
...
**Files:** Host environment when building `frontend/` (not automated in `install_device_ui.sh`)
...
| File | Purpose |
|---|---|
| `scripts/install_device_ui.sh` | Copies repo to `/opt/meetingbox`, systemd unit, Docker Compose up |
